...
const FlashcardSetCard = ({ flashcardSet }) => {
  ...
  const handleStudyNow = () => {
    navigate(`/documnets/${flashcardSet.documentId._id}/flashcards`);
  };

  const reviewedCount = flashcardSet.cards.filter(card => card.lastReviewed).length;
  const totalCards = flashcardSet.cards.legth;
  ...
  >
    <div className='space-y-4'>
      ...
      <div className='flex items-center gap-3 pt-2'>
        <div className='px-3 py-1.4 bg-slate-50 border border-slate-200 rounded-lg'>
