...
const QuizCard = ({quiz, onDelete}) => {
    return (
        <div className='group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-slate-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-slate-500/10 flex flex-col justify-between'>
            ...
            {/* Action Button */}
            <div className='mt-2 pt-4 border-t border-slate-400'>
                {quiz?.userAnswers?.length > 0 ? (
                    <Link to={`/quizzes/${quiz._id}/results`}>
                        <button className='group/btn w-full inline-flex items-center justify-center gap-2 h-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm rounded-xl transition-all duration-200 active:scale-95 cursor-pointer'>
