...
const FlashcardPage = () => {
  ...
  const [currentCardIndex, setCurrentCurdIndex] = useState(0);
  ...
  const handleNextCard = () => {
    ...
    setCurrentCurdIndex((prevIndex) => (prevIndex + 1) % flashcards.length)
  };
  ...
  const handleToggleStar = async (cardId) => {
    try {
      ...
      toast.error("Failed to update star stauts");
    }
  };
    ...
    <div>
      <div className='mb-4'>
        <Link
          ...
          className='inline-flex items-center gap-2 text-sm text-neutral-600 hiver:text-nuteral-900 transition-colors'>
          <ArrowLeft size={16} />
          Back to Document
        </Link>
      </div>
      ...
      <Modal
        ...
        onClose={() => setCurrentCurdIndex(false)}
        title="Confirm Delete Flashcard Set">

        <div className='space-y-4'>
          <p className='text-sm text-netural-600'>
            Are you sure want to delete all flashcards
          </p>
